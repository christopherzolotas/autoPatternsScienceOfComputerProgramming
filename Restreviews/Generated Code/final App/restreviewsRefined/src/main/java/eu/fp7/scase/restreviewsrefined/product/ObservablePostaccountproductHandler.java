...
public class ObservablePostaccountproductHandler{
	...
	//Observers List
    private ArrayList<JavaproductObserver> Observers = new ArrayList<JavaproductObserver>();

    public ObservablePostaccountproductHandler(int accountId, JavaproductModel oJavaproductModel, UriInfo oApplicationUri){
		...
		try{
			this.Observers = oHibernateController.getaccountObservers(Observers);
		}catch(Exception e){
	...
	public int registerToVerb(String observerReaction){
		JavaproductObserver oObserver;
		
		oObserver = new JavaproductObserver();
		oObserver.setObserverHTTPAction("POST");
		oObserver.setType("VERB");
		oObserver.setJavaproductModel(this.oJavaproductModel);
		oObserver.setAction(observerReaction);
		this.oHibernateController.createJavaproductObserver(oObserver);
	...
	public void unregister(String observerId){
		try{
			JavaproductObserver oObserver;
			for(JavaproductObserver observer : this.Observers){
				if(observer.getobserverId()==Integer.parseInt(observerId)){
					this.Observers.remove(this.Observers.indexOf(observer));
					this.oHibernateController.deleteJavaproductObserver(observer);
					break;
				}
			}
		}catch(Exception e){
	...
	public void notifyObservers(){
        for(JavaproductObserver o: Observers){
			if(o.getType().equals("VERB") && o.getObserverHTTPAction().equals("POST")){
				o.update("updating observer");
				this.oHibernateController.updateJavaproductObserver(o);
			}else{
				/* System.out.println(this.oJavaproductModel.getproductId());
				System.out.println(o.getJavaproductModel().getproductId()); */
				if(o.getJavaproductModel().getproductId() == this.oJavaproductModel.getproductId() && o.getObserverHTTPAction().equals("POST")){
